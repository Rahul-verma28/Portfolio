...
const Header = () => {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <h1 className="text-2xl dark:text-white hover:underline uppercase flex items-center gap-2"><Image src="/image.png" className=" rounded-full border-1 border-indigo-500 mr-dafoe-regular" width={40} height={40} alt="Logo" />Rahul.</h1>
        <div className="md:flex items-center space-x-3 md:space-x-8 hidden">
          {navItems.map((navItem: any, idx: number) => (
            ...
            >
              <span className="text-sm hover:text-violet-500 hover:font-bold">{navItem.name}</span>
            </Link>
          ))}
        </div>
